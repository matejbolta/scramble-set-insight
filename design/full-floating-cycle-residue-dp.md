...
Dokument obravnava **full floating**. Legacy `UFR / UF` tracing in partial
floating nista predmet predlagane poenostavitve, čeprav mora implementacija
ohraniti njuno obnašanje oziroma pravilno obravnavati omejen nabor bufferjev.
...
**Status:** exact full-floating algcount za weight `1` in custom orientation
weighte `>= 1` je implementiran v `web/cycle-residue.js` in
`web/cycle-residue-planner.js`. Residue katalog, concrete oracle in exhaustive
weighted class oracle so aktivni regresijski testi. Partial floating in
rekonstrukcija dejanskega optimized mema ostajata ločena nadaljnja koraka.
...
```text
pseudoswap: UF, UB, UR, UL, FR, FL, DF, DB, DR, DL
...
IRL katalog, full-buffer coverage in finish-aware algcount so implementirani.
Odprto ostaja:

1. Partial buffer set potrebuje lasten exact generator search; trenutni splošni
   DLin search ostaja transitional fallback.
2. Če želimo vrniti dejanski optimized memo, morajo residue prehodi dobiti
   cycle identitete, konkretne stickerje in backpointerje. Trenutni production
   rezultat namenoma vrne cost/residue dokaz, ne izmišljenega segment tracea.
...
- legacy `UFR / UF` mora ostati identičen frozen handwritten truthu;
...
cycle razredov; tako daljši cycle ne izgubi comm/orientation alternative. Odd
corner state dodatno uporablja vseh 416 UFR-rooted razredov in ločene Pareto
frontiere za `none`, `ltct` in `t2c`, zato je tudi izbira finisha eksaktna pri
poljubnem podprtem twist weightu.
Full-buffer coverage je dokazan, ne predpostavljen. Preostali odprti področji
sta partial buffer seti in rekonstrukcija konkretnega optimized mema.
